...
tidy_cannon <- function(data) {
  ...
  cols.char <-c("cond", "condcolor", "date", "time", "blockcode", "trialcode")

  data <- data %>% as_tibble() %>% mutate(across(-cols.char, ~as.numeric(.)))
  sapply(data, class)
  return(data)
}
...
create_vars <- function(data, set_vars){
  ...
  if("predErr" %in% set_vars) {
    ...
    for(r in 1:nrow(data)){
      ...
      data$predErr[r] <- discrep(as.numeric(data$outcome[r]), as.numeric(data$placementAngle[r]))}
      }
    }
