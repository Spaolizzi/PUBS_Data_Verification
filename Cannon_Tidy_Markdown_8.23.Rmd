...
set_vars <- c("Abs_PE", "PE_angmu", "catch_miss", "changepoint", "total_trialnum" ) ##name variables needed for analysis, add as they become apparent 
trim_cols <- TRUE
...
irreg_plot_hit <- cannon_data %>% filter(grepl("CHECK_HIT", Irreg)) %>% select(subject, cond, blocknum, trialnum, catch_miss, angmu, placementAngle, outcome, Abs_PE, angleup, angledown, shield_size)
irreg_plot_miss <- cannon_data %>% filter(grepl("CHECK_MISS", Irreg)) %>% select(subject, cond, blocknum, trialnum, catch_miss, angmu, placementAngle, outcome, Abs_PE, angleup, angledown, shield_size)
irreg_plot_NA <- cannon_data %>% filter(grepl("CHECK_NA", Irreg)) %>% select(subject, cond, blocknum, trialnum, catch_miss, angmu, placementAngle, outcome, Abs_PE, angleup, angledown, shield_size)
range <- cannon_data %>% 
...
**Figure 1**. Below is a histogram of all reaction time data.This is beautiful! Tried both woth and without subject 456, who seems a little problematic based on the summary statistics. Overall, happy with this!
...
for (i in subjects) {
  o <- cannon_data %>% dplyr::filter(subject == i & cond == "ODDBALL") %>% filter(!is.na(Abs_PE))
  if(nrow(o) > 100){
    oddballPE <- ggplot(o, aes(x=trialnum, y=Abs_PE, color = catch_miss)) + geom_point() +
  ...
  c <- cannon_data %>% dplyr::filter(subject == i & cond == "CHANGEPOINT") %>% filter(!is.na(Abs_PE))
  if(nrow(c) > 100){
...
rt_trialnum <- lm(trial.placeshield_mouse.latency ~ total_trialnum + cond, data = cannon_data)
summary(rt_trialnum)
