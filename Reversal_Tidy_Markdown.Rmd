...
pacman::p_load(tidyverse, readr, janitor, ggplot2,wesanderson, cowplot, flextable, emmeans, data.table)
...
distinct <- reversal %>%
  ...
  mutate(trial_response = ifelse(rightleftcorrect == "left", trial.presentation_left.response, trial.presentation_right.response)) %>% ## combine responses to create overall trial response
  ...
  select(!c(trial.presentation_left.latency, trial.presentation_right.latency, trial.presentation_left.response, stimulusitem1, trial.presentation_right.response)) #%>% # delete extra columns for conciseness
  #filter(!total_trialnum > 240 && !block_number > 3) %>% filter(!trial_number == 0) %>% #removing things we don't need
  #group_by(subject, block_number) %>% mutate(percent_correct_block = as.numeric(numbercorrectfeedback)/80) %>% ##percentage correct across block
```
