...
```{r, include=FALSE}
##long pipe for creating variables
reversal <- reversal %>%
...
mutate(ResponseCorrect = as.numeric(isResponseCorrect)) %>% ## transform to numeric
  ...
  ungroup() %>% select(-grp)
  # delete extra columns for conciseness
...
plots <- lapply(1:length(rts), function(z) {knitr::knit_expand(text = template)})
...
for (i in subjects) {
  s <- reversal %>% dplyr::filter(subject == i)
  scatter <- ggplot(s, aes(x=trial_number, y= trial_response, color = as.factor(isResponse_num))) +
     geom_point() +
