...
reversal_data <- data.table::fread("~/github_repos/ReversalTask/Reversal_pilot_mTurk.csv", fill = TRUE) %>% group_by(subject, time) %>% filter(n() >= 200)
...
```
A few critical concerns come up immediately: First, there are IDs that overlap. This was updated in the qualtrics survey, and luckily data was still distinguishable by time stamp. Second, one of those repeated subject did not complete the task fully (they bailed in the middle of the cannon task). - It's unclear why, but this means I cannot pay this participant. They also only completed 40% of the surveys, indicating to me they were not going to receive payment anyway. Third, it seems the workerIDs were not recorded in the qualtrics survey. This has now been updated and will be testing this evening. 
...
rts[]
summary(reversal$trial_latency)
```
...
Overall, these graphs corroborate the idea that participants may be learning incredibly quickly. I worry that this means participants will reach a ceiling very fast, and we won't see effects of block or learning over time. Below, I run a few quick tests to see if 

```{r}
a_1 <- aov(trial_latency ~ task_phase, data = reversal)
summary(a_1)
a_1 <- aov(trial_latency ~ block_number, data = reversal)
summary(a_1)

rt_trialnum <- lm(trial_latency ~ total_trialnum + task_phase + block_number, data = reversal)
summary(rt_trialnum)
...
11 participants made the "correct" choice approximately 85% of the time, and made at least 10 correct choices by trial 12, on average. Both of these outcomes suggest the block length can be shortened to as little as 5 blocks of 40 trials. 
